...
import { api, errMsg, money, fmtDate, PRESTATION_STATUS } from '../api.js';
    ...
    <div className="page container narrow">
      ...
      {isClient && p.status === 'COMPLETED' && !p.payment ? (
        <section className="section-box">
          ...
          <p className="muted">Paiement direct après la prestation. Montant : <strong>{money(p.amount)}</strong></p>
          <div className="form">
            <label>
              Moyen de paiement
          ...
          {initiated ? (
            <div className="sandbox-box">
              <Alert tone="info">
                <strong>Mode démo (sandbox)</strong> — aucun débit réel.
                <div className="muted small">{initiated.instructions}</div>
              </Alert>
              <div className="form">
                <label>
                  Code de confirmation (affiché en démo)
        ...
        <section className="section-box">
          ...
          <div className="form">
            ...
            <button type="button" className="btn btn-primary" disabled={confirming} onClick={() => confirm(p.payment.id)}>
              {confirming ? 'Confirmation…' : 'Reprendre la confirmation'}
            </button>
          </div>
        </section>
      ) : null}
